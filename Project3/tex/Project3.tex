...
\\
We also found it interesting to explore how things would change with different gravitational forces. We know Newton's force of gravitation is proportional to one over $r$ squared ($F\propto1/r^2$). Where $r$ is the distance between two objects. We want to know how things would change if the force was proportional to ex. $1/r^3$. Thus we also look at different gravitational forces proportional to $1/r^n$, where $n\in\{2,3\}$.
\\
...
\end{align*}
From equation \eqref{eq:Taylor_exp_x} we see that the mathematical error in this approximation goes like $O(h^3)$.
...
\begin{align*}
  ...
  &= \text{constant}
\end{align*}
Angular momentum is conserved. This holds true for all the bodies in a many-body system where the objects interact only through the gravitational force. In this case we can settle for calculating only the total angular momentum of all the bodies, which should be conserved. This is done by summing over all the bodies in the system:
\begin{equation*}
...
Newton's gravitational force is conservative, and the energy of our system should therefore be conserved. Kinetic energy is given by
\begin{equation}
...
\begin{equation}
  ...
  U = - G \frac{m M}{r}
\end{equation}
Where $U$ is the potential energy, $G$ the gravitational constant, $r$ the relative distance between the objects.

In a many-body system of celestial bodies interacting only through the gravitational force the total energy should be conserved. The total energy is the sum of the kinetic and potential energy of all the bodies in the system:
\begin{align*}
...
The sum of the potential energy $U$ and the kinetic energy $K$ will be conserved in a system of celestial bodies. If we consider the idealized case of an object with mass $m \ll M_\odot$ in orbit around the sun we can In order to escape the gravitational pull of the Sun, a planet in orbit must have a total energy $E = U_G + K \ge 0$ as the potential energy goes to zero infinitely far away:
\begin{align*}
  E = U_G + K &\ge 0 \\
  K &\ge - U_G \\
