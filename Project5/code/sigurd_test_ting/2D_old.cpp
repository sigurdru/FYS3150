...
TwoDimensions::TwoDimensions(Parameters params, double** InitialConditions) {
  Initialize(params);
...
#pragma omp parallel for schedule(static, 1)
  for (int l = 0; l < Nt; l++) {
    ...
    for (int i = 0; i < Nx; i++) {
      while (good_to_go[cc][i] != 1)
        usleep(5);
      for (int j = i; j > 1; j--) {
        u[cc + 1][i][j] =
            u[cc][i][j] +
            alpha * (u[cc][i + 1][j] + u[cc][i - 1][j] + u[cc][i][j + 1] +
